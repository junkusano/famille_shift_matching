...
import { Plus, Trash2, Save, RefreshCcw, Eye, Calendar as CalIcon, ChevronDown } from "lucide-react";
...
async function apiFetchTemplates(cs: string): Promise<TemplateRow[]> {
    ...
    return (data?.rows ?? []).map((r: any) => ({ ...r, _cid: crypto.randomUUID(), _selected: false }));
}
...
const TableHeadCell: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    <th className="text-left text-xs font-semibold text-slate-500 px-2 py-2 border-b">{children}</th>
);

const TableCell: React.FC<{ children: React.ReactNode }> = ({ children }) => (
    <td className="px-2 py-2 align-top border-b">{children}</td>
);
...
export default function WeeklyRosterPage() {
    ...
    async function saveAll() {
        const allErrs = rows.flatMap((r, idx) => validateRow(r).map((m) => `#${idx + 1}: ${m}`));
        if (duplicate) {
        ...
        try {
            const payload = rows.map(({ _cid, _selected, ...r }) => r);
