...
function nowYYYYMM() {
    const d = new Date();
    // toLocaleString keeps TZ for consistency
...
function validateRow(r: TemplateRow): string[] {
    ...
    if (re.test(r.start_time) && re.test(r.end_time)) {
        if (r.end_time <= r.start_time) errs.push("終了時刻は開始より後である必要があります");
    }
    ...
    const rolesOk = (x?: string | null) => !x || ["-999", "01", "02"].includes(x);
...
// =========================
// API wrappers (adjust endpoints to your backend)
// =========================
async function apiFetchTemplates(cs: string): Promise<TemplateRow[]> {
    ...
    return (data?.rows ?? []).map((r) => ({ ...r, _cid: crypto.randomUUID(), _selected: false }));
}

async function apiBulkUpsert(rows: TemplateRow[]) {
...
// UI Components
// =========================
const Pill: React.FC<{ label: string; tone?: "ok" | "warn" | "muted" }>
    = ({ label, tone = "muted" }) => (
        <span className={[
...
const TableHeadCell: React.FC<React.ThHTMLAttributes<HTMLTableCellElement>> = ({ children, className, ...rest }) => (
    <th {...rest} className={["text-left text-xs font-semibold text-slate-500 px-2 py-2 border-b", className].filter(Boolean).join(" ")}>{children}</th>
);

const TableCell: React.FC<React.TdHTMLAttributes<HTMLTableCellElement>> = ({ children, className, ...rest }) => (
    <td {...rest} className={["px-2 py-2 align-top border-b", className].filter(Boolean).join(" ")}>{children}</td>
);
...
export default function WeeklyRosterPage() {
    ...
    function addRow() {
        if (!cs) { alert("先に利用者ID(kaipoke_cs_id)を入力してください"); return; }
        setRows((rs) => [...rs, newEmptyRow(cs)]);
    }
    ...
    function removeSelected() {
        const idsToDelete = rows.filter(r => r._selected && r.template_id).map(r => r.template_id!)
        const remaining = rows.filter(r => !r._selected);
        if (idsToDelete.length === 0) { setRows(remaining); return; }
    ...
    async function load() {
        if (!cs) { alert("利用者IDを入力してください"); return; }
...
保存を続行しますか？`)) return;
        ...
        try {
            const payload = rows.map((r) => {
                const { _cid, _selected, ...rest } = r;
                void _cid; void _selected; // intentionally unused (strip client-only fields)
                return rest;
            });
            await apiBulkUpsert(payload as TemplateRow[]);
            await load();
        } catch (e: unknown) {
            const msg = e instanceof Error ? e.message : String(e);
            setError(msg);
        ...
        <div className="p-6 space-y-4">
            ...
            <div className="flex flex-wrap items-end gap-3">
                ...
                <div className="flex items-center gap-2">
                    <ToolbarButton onClick={load} disabled={!cs || loading}
                        icon={<RefreshCcw className="w-4 h-4" />}>読み込み</ToolbarButton>
                    <ToolbarButton onClick={addRow} disabled={!cs}
                        icon={<Plus className="w-4 h-4" />}>行を追加</ToolbarButton>
                    <ToolbarButton onClick={removeSelected} disabled={rows.every(r => !r._selected)}
                        icon={<Trash2 className="w-4 h-4" />}>選択削除</ToolbarButton>
                    <ToolbarButton onClick={saveAll} disabled={!rows.length || saving || duplicate}
                        icon={<Save className="w-4 h-4" />}>保存</ToolbarButton>
                    <ToolbarButton onClick={doPreview} disabled={!cs || loading}
                        icon={<Eye className="w-4 h-4" />}>月展開プレビュー</ToolbarButton>
                </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
                ...
                {duplicate && <Pill tone="warn" label="重複 (同曜日×開始時刻) あり" />}
                {invalidCount > 0 && <Pill tone="warn" label={`警告 ${invalidCount}件`} />}
                {useRecurrence
                    ? <Pill tone="ok" label="隔週/第n週 有効" />
                    : <Pill tone="muted" label="隔週/第n週 無効" />}
                ...
                <div className="overflow-auto rounded-2xl border">
                    <table className="min-w-full text-sm">
                        ...
                        <tbody>
                                    ...
                                    <tr key={r._cid} className={errs.length ? "bg-amber-50" : ""}>
                                        <TableCell>
                                            <input type="checkbox" checked={!!r._selected}
                                                onChange={(e) => updateRow(r._cid!, { _selected: e.target.checked })} />
                                        </TableCell>
                                        <TableCell>
                                            <select value={r.weekday} onChange={(e) => updateRow(r._cid!, { weekday: Number(e.target.value) })}
                                        ...
                                        <TableCell>
                                            <div className="flex items-center gap-1">
                                                <input type="time" value={r.start_time}
                                                    onChange={(e) => updateRow(r._cid!, { start_time: e.target.value })}
                                                    className="border rounded-lg px-2 py-1" />
                                                <span className="text-slate-400">〜</span>
                                                <input type="time" value={r.end_time}
                                                    onChange={(e) => updateRow(r._cid!, { end_time: e.target.value })}
                                                    className="border rounded-lg px-2 py-1" />
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                            <input value={r.service_code}
                                                onChange={(e) => updateRow(r._cid!, { service_code: e.target.value })}
                                                placeholder="身体/重訪Ⅱ など"
                                                className="border rounded-lg px-2 py-1 w-36" />
                                        </TableCell>
                                        <TableCell>
                                            <div className="flex items-center gap-2">
                                                <input type="number" min={1} value={r.required_staff_count}
                                                    onChange={(e) => updateRow(r._cid!, { required_staff_count: Number(e.target.value) })}
                                                    className="border rounded-lg px-2 py-1 w-16" />
                                                <label className="inline-flex items-center gap-1 text-xs">
                                                    <input type="checkbox" checked={r.two_person_work_flg}
                                                        onChange={(e) => updateRow(r._cid!, { two_person_work_flg: e.target.checked })} />
                                                    2人従事
                                                </label>
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                            <input value={r.judo_ido ?? ""}
                                                onChange={(e) => updateRow(r._cid!, { judo_ido: e.target.value || null })}
                                                placeholder="例: 0015"
                                        ...
                                        <TableCell>
                                            <div className="flex flex-col gap-1">
                                                <div className="flex items-center gap-1">
                                                    <input value={r.staff_01_user_id ?? ""} onChange={(e) => updateRow(r._cid!, { staff_01_user_id: e.target.value || null })}
                                                        placeholder="staff1" className="border rounded-lg px-2 py-1 w-32" />
                                                    <span className="text-xs text-slate-400">出動</span>
                                                </div>
                                                <div className="flex items-center gap-1">
                                                    <input value={r.staff_02_user_id ?? ""} onChange={(e) => updateRow(r._cid!, { staff_02_user_id: e.target.value || null })}
                                                        placeholder="staff2" className="border rounded-lg px-2 py-1 w-32" />
                                                    <label className="text-xs inline-flex items-center gap-1">
                                                        <input type="checkbox" checked={r.staff_02_attend_flg}
                                                            onChange={(e) => updateRow(r._cid!, { staff_02_attend_flg: e.target.checked })} /> 同行
                                                    </label>
                                                </div>
                                                <div className="flex items-center gap-1">
                                                    <input value={r.staff_03_user_id ?? ""} onChange={(e) => updateRow(r._cid!, { staff_03_user_id: e.target.value || null })}
                                                        placeholder="staff3" className="border rounded-lg px-2 py-1 w-32" />
                                                    <label className="text-xs inline-flex items-center gap-1">
                                                        <input type="checkbox" checked={r.staff_03_attend_flg}
                                                            onChange={(e) => updateRow(r._cid!, { staff_03_attend_flg: e.target.checked })} /> 同行
                                                    </label>
                                                </div>
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                            <div className="flex flex-col gap-1">
                                                <input value={r.staff_01_role_code ?? ""}
                                                    onChange={(e) => updateRow(r._cid!, { staff_01_role_code: e.target.value || null })}
                                                    placeholder="-999/01/02" className="border rounded-lg px-2 py-1 w-24" />
                                                <input value={r.staff_02_role_code ?? ""}
                                                    onChange={(e) => updateRow(r._cid!, { staff_02_role_code: e.target.value || null })}
                                                    placeholder="-999/01/02" className="border rounded-lg px-2 py-1 w-24" />
                                                <input value={r.staff_03_role_code ?? ""}
                                                    onChange={(e) => updateRow(r._cid!, { staff_03_role_code: e.target.value || null })}
                                                    placeholder="-999/01/02" className="border rounded-lg px-2 py-1 w-24" />
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                            <div className="flex items-center gap-1">
                                                <input type="date" value={r.effective_from ?? ""}
                                                    onChange={(e) => updateRow(r._cid!, { effective_from: e.target.value || null })}
                                                    className="border rounded-lg px-2 py-1" />
                                                <span className="text-slate-400">〜</span>
                                                <input type="date" value={r.effective_to ?? ""}
                                                    onChange={(e) => updateRow(r._cid!, { effective_to: e.target.value || null })}
                                                    className="border rounded-lg px-2 py-1" />
                                            </div>
                                        </TableCell>
                                        <TableCell>
                                            <div className="flex flex-col gap-1">
                                                <label className="text-xs inline-flex items-center gap-1">
                                                    <input type="checkbox" checked={!!r.is_biweekly}
                                                        onChange={(e) => updateRow(r._cid!, { is_biweekly: e.target.checked })} /> 隔週
                                                </label>
                                                <input
                                                    value={(r.nth_weeks ?? []).join(",")}
                                                    onChange={(e) => updateRow(r._cid!, { nth_weeks: e.target.value ? e.target.value.split(",").map(x => Number(x.trim())).filter(Boolean) : [] })}
                                        ...
                                        <TableCell>
                                            <label className="inline-flex items-center gap-2">
                                                <input type="checkbox" checked={r.active}
                                                    onChange={(e) => updateRow(r._cid!, { active: e.target.checked })} />
                                            ...
                                            {errs.length > 0 && (
                                                <div className="mt-1 text-[11px] text-amber-700 space-y-0.5">
                                                    ...
                                                    {errs.length > 3 && <div>…他{errs.length - 3}件</div>}
                                                </div>
                                            )}
                                        </TableCell>
                                    </tr>
                            ...
                            {rows.length === 0 && (
                                <tr>
                                    <td className="text-center text-slate-400 py-8" colSpan={11}>テンプレ行がありません。「読み込み」または「行を追加」をクリック</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
                                    ...
                                    {preview.map((p, i) => {
                                        ...
                                        return (
                                            <tr key={i} className="border-b">
                                                ...
                                                <TableCell>{p.shift_start_time.substring(0, 5)}〜{p.shift_end_time.substring(0, 5)}</TableCell>
            ...
            <div className="text-xs text-slate-500 space-y-1">
                <div>・この画面は <strong>週間テンプレ（shift_weekly_template）</strong> の編集専用です。月間シフト（shift）には直接書き込みません。</div>
