    ...
    return (
        <div className="flex portal-container min-h-screen">
            ...
            <div className={`menu ${isMenuOpen ? 'open' : ''}`}>
                <h2 className="text-xl font-semibold ml-4">{userData.last_name_kanji} {userData.first_name_kanji}</h2>
                <p className="text-white font-semibold text-sm ml-4 mt-1 drop-shadow-sm">ユーザー権限: {role}</p>
                <div className="relative w-32 h-32">
                    {userData.photo_url ? (
                        <>
                            <Image src={userData.photo_url} width={128} height={128} alt="写真" className="rounded-full object-cover w-full h-full" />
                            <button className="absolute bottom-0 right-0 bg-red-500 text-white text-xs px-1 py-0.5 rounded hover:bg-red-600" onClick={handleDeletePhoto}>×</button>
                        </>
                    ) : (
                        <label className="flex flex-col items-center justify-center w-full h-full bg-gray-300 text-gray-600 text-sm rounded-full cursor-pointer">
                            Upload
                            <input type="file" accept="image/*" onChange={handlePhotoReupload} className="hidden" />
                        </label>
                    )}
                </div>
                <ul className="mt-6 ml-4 space-y-2">
                    ...
                    {(role === 'manager' || role === 'admin') && <li><Link href="/portal/entry-list" className="text-blue-300 hover:underline">エントリー一覧</Link></li>}
                    ...
                    <li><Link href="/portal/shift" className="text-blue-300 hover:underline">シフト（お休み希望も）</Link></li>
                    ...
                    <li><Link href="/lineworks-login-guide" className="hover:underline" target="_blank" rel="noopener noreferrer">LINE WORKSログインガイド</Link></li>
                </ul>
                <div className="ml-4 mt-4">
                    <hr className="border-white my-2" />
                    <button onClick={async () => { await supabase.auth.signOut(); router.push('/'); }} className="text-sm text-red-300 hover:underline">🚪 ログアウト</button>
                    <hr className="border-white my-2" />
                </div>
            </div>
            <div className="left-menu flex flex-col justify-between h-full min-h-screen">
                ...
                <div className="pt-4">
                    <hr className="border-white my-2" />
                    <button onClick={async () => { await supabase.auth.signOut(); router.push('/'); }} className="text-sm text-red-500 hover:underline">🚪 ログアウト</button>
                    <hr className="border-white my-2" />
                </div>
            </div>
