...
export default function RpaCommandTemplateListPage() {
  ...
  const handleUpdate = async () => {
    if (!editId) return;
    const { data, error } = await supabase.from('rpa_command_templates')
      .update({
        name: editName,
          ...
          <Card key={template.id}>
            <CardContent className="p-4 flex justify-between items-center">
              ...
              <div className="flex gap-2">
                <Button variant="outline" onClick={() => startEdit(template)}>編集</Button>
                <Button variant="destructive" onClick={() => handleDelete(template.id)}>削除</Button>
              </div>
            </CardContent>
          </Card>
        ))}
