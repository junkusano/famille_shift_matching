  ...
  return (
    <main className="mx-auto max-w-7xl space-y-6 p-4 text-slate-900 md:p-6">
      ...
      <section className="space-y-3">
        ...
        <div className="grid gap-4 lg:grid-cols-2">{fixedPlaybooks.map((playbook) => <article key={playbook.id} className="rounded-xl border border-indigo-200 bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-bold">{playbook.name}</h3><span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${playbook.is_enabled ? "bg-emerald-100 text-emerald-800" : "bg-slate-200 text-slate-700"}`}>{playbook.is_enabled ? "稼働中" : "停止中"}</span></div><p className="mt-1 text-sm text-slate-600">{playbook.description}</p></div><LockKeyhole className="shrink-0 text-indigo-600" size={20} aria-label="固定処理" /></div><dl className="mt-4 grid gap-3 rounded-lg bg-slate-50 p-3 text-sm sm:grid-cols-2"><div><dt className="text-xs font-semibold text-slate-500">開始</dt><dd className="mt-0.5">{TRIGGER_MODE_LABELS[playbook.trigger_mode]}</dd></div><div><dt className="text-xs font-semibold text-slate-500">処理方式</dt><dd className="mt-0.5">{EXECUTION_MODE_LABELS[playbook.execution_mode]}</dd></div></dl><div className="mt-3 text-sm"><span className="font-semibold">処理：</span>{playbook.instructions}</div></article>)}</div>
      </section>
