...
export default function TaimeeEmployeesPage() {
  ...
  async function onUpload(e: React.FormEvent) {
    ...
    try {
      ...
      // もう period は送らない → アップロードフォームも月入力を削除
      // CSVから取り込み月を別管理したい場合は、API側でファイル名などから判定してください
      fd.append('period', '')
      fd.append('file', file)
  ...
  }, [items, fPeriod, fLast, fFirst, fPhone])
        ...
        <CardContent className="p-4 space-y-4">
          <div className="flex flex-col md:flex-row gap-2 md:items-center md:justify-between">
            <div className="flex gap-2 items-center">
              <span className="text-sm">在籍：</span>
            ...
            <table className="min-w-[880px] text-sm">
              <thead className="bg-muted">
                ...
                <tr className="border-t">
                  ...
                  <th className="p-2 text-xs text-muted-foreground">上のセレクトで切替</th>
                  <th className="p-2"><Button variant="outline" size="sm" onClick={() => { setFPeriod(''); setFLast(''); setFFirst(''); setFPhone('') }}>クリア</Button></th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((it) => {
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={6} className="p-6 text-center text-muted-foreground">データがありません</td></tr>
                )}
              </tbody>
            </table>
          </div>
