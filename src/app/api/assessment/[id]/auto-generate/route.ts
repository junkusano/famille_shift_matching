...
// ===== Assessment Content Types（any禁止）=====
type AssessmentRow = {
  key: string;
  label: string;
  check: "NONE" | "CIRCLE";
...
// summaryが短いと「材料不足」になるので、短いときはOCRも合体する
function pickText(row: CsDocRow, maxOcr: number) {
  const summary = trimOrEmpty(row.summary);
  const ocr = trimOrEmpty(row.ocr_text);

  // summaryが短い場合は summary + ocr で材料を増やす
  if (summary && summary.length < 800 && ocr) {
    return {
      text: `${summary}\n\n--- OCR ---\n${take(ocr, maxOcr)}`,
      use: "summary+ocr" as const,
    };
  }
...
export async function POST(req: NextRequest, { params }: Ctx) {
  try {
    ...
    // assessment.content は template と同型のはず
    ...
    const corePicked = CORE_DOC_NAMES.map((n) => pickLatestByName(n, 12000)).filter(
...
- template_content の sheets/rows は「必ず全て含める」（省略禁止）
- まず template_content を丸ごとコピーし、remark/hope/check だけ更新するイメージで出力する
...
`.trim();
    ...
    console.log("[assessment:auto-generate] openai raw", {
      ...
      content_chars: (resp.choices?.[0]?.message?.content ?? "").length,
    });

    const txt = resp.choices?.[0]?.message?.content ?? "";
    if (!txt.trim()) throw new Error("OpenAI returned empty content");

    console.log("[assessment:auto-generate] openai head", {
      assessment_id: id,
