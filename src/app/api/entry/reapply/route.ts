...
const contactMessage = () => {
  const phone = process.env.SMS_MAIN_PHONE || process.env.NEXT_PUBLIC_SMS_MAIN_PHONE;
  const email = process.env.RECRUIT_CONTACT_EMAIL;
  const contacts = [phone ? `電話：${phone}` : "", email ? `メール：${email}` : ""].filter(Boolean).join(" / ");
  return contacts ? `うまく受付できない場合は、採用担当者までご連絡ください（${contacts}）。` : "うまく受付できない場合は、採用担当者までご連絡ください。";
};

export async function POST(req: NextRequest) {
  try {
    const body = await req.json();
    const required = [body.lastNameKanji, body.firstNameKanji, body.lastNameKana, body.firstNameKana, body.birthYear, body.birthMonth, body.birthDay, body.phone, body.email, body.reapplyReason];
    if (required.some((v) => !String(v ?? "").trim())) return NextResponse.json({ ok: false, message: `${publicNotFound}\n${contactMessage()}` }, { status: 400 });
    const reapplyReason = String(body.reapplyReason).trim();
    ...
    const { error: logError } = await supabaseAdmin.from("staff_log").insert({ staff_id: entry.id, action_at: new Date().toISOString(), action_detail: `再応募申請\n理由：${reapplyReason}`, registered_by: "reapply_form" });
    ...
    return NextResponse.json({ ok: false, message: `再応募の受付を完了できませんでした。時間をおいてお試しください。\n${contactMessage()}` }, { status: 500 });
  }
}
