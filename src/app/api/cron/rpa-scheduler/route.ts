...
type Definition = { id: string; name: string; job_type: string; schedule: { timezone?: unknown; times?: unknown }; payload: Record<string, unknown> };
...
export async function GET(request: NextRequest) {
  ...
  const now = jstMinute();
  const { data, error } = await supabaseAdmin.from('rpa_job_definitions').select('id,name,job_type,schedule,payload').eq('is_enabled', true).eq('trigger_type', 'schedule').eq('execution_mode', 'famille_rpa');
  ...
  for (const definition of (data ?? []) as Definition[]) {
    ...
    const { error: insertError } = await supabaseAdmin.from('rpa_runner_jobs').insert({ job_type: definition.job_type, payload: definition.payload, status: 'pending', job_definition_id: definition.id, scheduled_for: now.scheduledFor });
