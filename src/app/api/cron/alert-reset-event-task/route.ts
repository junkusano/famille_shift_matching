...
export async function GET(req: NextRequest) {
    try {
        ...
        console.info('[cron][alert-reset-event-task] start');
        ...
        const { data: openRows, error: openError } = await supabaseAdmin
            ...
            .like('message', '%【イベントタスク期限超過】%');

        if (openError) {
            console.error('[cron][alert-reset-event-task] openRows error detail', {
        ...
        for (const row of openRows ?? []) {
            ...
            if (doneError) {
                console.error('[cron][alert-reset-event-task] doneRow error detail', {
            ...
            if (updateError) {
                console.error('[cron][alert-reset-event-task] update error detail', {
        ...
        console.info('[cron][alert-reset-event-task] end', {
        ...
        console.error('[cron][alert-reset-event-task] error', msg);
