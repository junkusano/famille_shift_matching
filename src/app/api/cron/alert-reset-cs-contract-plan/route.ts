...
export async function GET(req: NextRequest) {
    try {
        ...
        console.info('[cron][alert-reset-cs-contract-plan] start');
        ...
        const { data: openRows, error: openError } = await supabaseAdmin
            ...
            .like('message', '%書類の作成＆サイン受領を実施してください。%');

        if (openError) {
            console.error('[cron][alert-reset-cs-contract-plan] openRows error detail', {
        ...
        for (const row of openRows ?? []) {
            ...
            if (doneError) {
                console.error('[cron][alert-reset-cs-contract-plan] doneRow error detail', {
            ...
            if (updateError) {
                console.error('[cron][alert-reset-cs-contract-plan] update error detail', {
        ...
        console.info('[cron][alert-reset-cs-contract-plan] end', {
        ...
        console.error('[cron][alert-reset-cs-contract-plan] error', msg);
