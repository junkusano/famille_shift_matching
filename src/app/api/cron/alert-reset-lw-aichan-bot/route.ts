...
export async function GET(req: NextRequest) {
    try {
        ...
        console.info('[cron][alert-reset-lw-aichan-bot] start');
        ...
        const { data: openRows, error: openError } = await supabaseAdmin
            ...
            .like('message', '%【LW Bot未登録】%');

        if (openError) {
            console.error('[cron][alert-reset-lw-aichan-bot] openRows error detail', {
        ...
        for (const row of openRows ?? []) {
            ...
            if (doneError) {
                console.error('[cron][alert-reset-lw-aichan-bot] doneRow error detail', {
            ...
            if (updateError) {
                console.error('[cron][alert-reset-lw-aichan-bot] update error detail', {
        ...
        console.info('[cron][alert-reset-lw-aichan-bot] end', {
        ...
        console.error('[cron][alert-reset-lw-aichan-bot] error', msg);
