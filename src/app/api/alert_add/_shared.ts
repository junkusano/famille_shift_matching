...
export function getIncomingCronToken(req: NextRequest): { token: string | null; src: 'query'|'header'|'auth'|'none' } {
  ...
  if (auth && auth.toLowerCase().startsWith('bearer ')) return { token: auth.slice(7).trim(), src: 'auth' };
...
export function assertCronAuth(req: NextRequest) {
  ...
  const mask = (s?: string | null) => (s ? `${s.slice(0,2)}...(${s.length})` : 'null');
...
export async function ensureSystemAlert(params: EnsureAlertParams): Promise<EnsureResult> {
  const {
    message, severity = 2, visible_roles = ['manager', 'staff'],
    kaipoke_cs_id = null, user_id = null, shift_id = null, rpa_request_id = null,
  } = params;
  ...
  const { data: exists, error: selErr } = await supabaseAdmin
    ...
    .eq('message', message)
    .limit(1);
  if (selErr) { console.error('[alert][ensure] select error', selErr); throw selErr; }
  if (exists?.length) {
  ...
  const { data: inserted, error: insErr } = await supabaseAdmin
    .from('alert_log')
    .insert({
      message, visible_roles, status: 'open', status_source: 'system', severity,
      kaipoke_cs_id, user_id, shift_id, rpa_request_id,
    })
    .select('id')
    .single();
  if (insErr) { console.error('[alert][ensure] insert error', insErr); throw insErr; }
