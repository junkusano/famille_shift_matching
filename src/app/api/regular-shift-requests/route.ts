...
async function actor() {
  ...
  const { data } = await supabaseAdmin.from('users').select('user_id,lw_userid,manager_lw_userid').eq('auth_user_id', auth.user.id).maybeSingle();
  return data ? { ...data, authUserId: auth.user.id } : null;
}

function sameUser(row: ShiftRow, userId: string) {
  return [row.staff_01_user_id, row.staff_02_user_id, row.staff_03_user_id].includes(userId);
}

async function findCandidates(source: ShiftRow, userId: string) {
  const { data, error } = await supabaseAdmin.from('shift')
    .select('shift_id,kaipoke_cs_id,shift_start_date,shift_start_time,shift_end_time,service_code,staff_01_user_id,staff_02_user_id,staff_03_user_id,name')
    .eq('kaipoke_cs_id', source.kaipoke_cs_id)
    .eq('service_code', source.service_code)
    .eq('shift_start_time', source.shift_start_time)
    .eq('shift_end_time', source.shift_end_time)
    .order('shift_start_date', { ascending: false })
    .limit(500);
  if (error) throw error;
  const requests = await supabaseAdmin.from('regular_shift_requests').select('weekly_shift_id').eq('user_id', userId).neq('status', 'cancelled');
  if (requests.error) throw requests.error;
  const requested = new Set((requests.data ?? []).map((row) => String(row.weekly_shift_id)));
  return ((data ?? []) as ShiftRow[])
    .filter((row) => weekdayOf(row.shift_start_date) === weekdayOf(source.shift_start_date))
    .filter((row) => sameUser(row, userId))
    .filter((row) => timeOverlaps(row, source))
    .slice(0, 20)
    .map((row) => ({
      weekly_shift_id: String(row.shift_id), shift_start_date: row.shift_start_date,
      shift_start_time: row.shift_start_time, shift_end_time: row.shift_end_time,
      service_code: row.service_code, client_name: row.name ?? null,
      recurring_label: '週間シフト', requested: requested.has(String(row.shift_id)),
    }));
}
...
export async function POST(req: Request) {
  try {
    ...
    const { data: source } = await supabaseAdmin.from('shift').select('*').eq('shift_id', body.source_shift_id).maybeSingle();
    const { data: weekly } = await supabaseAdmin.from('shift').select('*').eq('shift_id', body.weekly_shift_id).maybeSingle();
    ...
    if (!candidates.some((candidate) => candidate.weekly_shift_id === String(weekly.shift_id))) return NextResponse.json({ error: 'weekly shift is not eligible' }, { status: 400 });
    const { data: saved, error } = await supabaseAdmin.from('regular_shift_requests').insert({
      user_id: String(me.user_id), weekly_shift_id: Number(body.weekly_shift_id), source_shift_id: Number(body.source_shift_id),
      available_from_month: regularStartMonth(), status: 'requested',
    }).select('id').single();
    if (error) {
    ...
    try {
      ...
      if (channel?.channel_id) {
        ...
        const text = `<m userId="${me.manager_lw_userid ?? me.lw_userid ?? ''}">マネジャー</m>\nレギュラーシフト希望が登録されました。\n希望者: <m userId="${me.lw_userid ?? ''}">さん</m>\n週間シフト: ${weekly.shift_start_date} ${String(weekly.shift_start_time).slice(0, 5)}〜${String(weekly.shift_end_time).slice(0, 5)}\n対象シフト: ${source.shift_start_date} ${String(source.shift_start_time).slice(0, 5)}〜${String(source.shift_end_time).slice(0, 5)}\n利用者: ${source.name ?? '不明'}\nレギュラー開始可能: ${regularStartMonth().slice(0, 7)}`;
        await sendLWBotMentionMessage({ botId: process.env.LINEWORKS_BOT_NO || process.env.WORKS_BOT_NO || '6807751', channelId: String(channel.channel_id), accessToken: await getAccessToken(), mentions, buildText: () => text });
        notified = true;
        await supabaseAdmin.from('regular_shift_requests').update({ lineworks_notified_at: new Date().toISOString() }).eq('id', saved.id);
      }
    } catch (notifyError) { console.error('[regular-shift-requests][notify]', notifyError); }
