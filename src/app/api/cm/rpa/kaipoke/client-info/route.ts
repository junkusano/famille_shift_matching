...
export const POST = cmRpaApiHandler<ApiResponse>(
  "cm/api/rpa/kaipoke/client-info",
  async (request, logger) => {
    ...
    if (basic_info) {
      ...
      if (basic_info.client_status)
        infoData.client_status = basic_info.client_status;
      if (basic_info.contract_date)
        infoData.contract_date = basic_info.contract_date;
      if (basic_info.biko) infoData.biko = basic_info.biko;

      const success = await cmUpsertRecord(
        "cm_kaipoke_info",
        infoData,
        { kaipoke_cs_id },
        "基本情報: ",
    ...
    // 被保険者証情報のUPSERT（cm_kaipoke_insurance）
    if (insurance_list && insurance_list.length > 0) {
      for (const insurance of insurance_list) {
        ...
        if (insurance.coverage_start)
          insuranceData.coverage_start = insurance.coverage_start;
        if (insurance.coverage_end)
          insuranceData.coverage_end = insurance.coverage_end;
        if (insurance.insurer_code)
          insuranceData.insurer_code = insurance.insurer_code;
        if (insurance.insurer_name)
          insuranceData.insurer_name = insurance.insurer_name;
        if (insurance.cert_status)
          insuranceData.cert_status = insurance.cert_status;
        if (insurance.insured_number)
          insuranceData.insured_number = insurance.insured_number;
        if (insurance.issue_date)
          insuranceData.issue_date = insurance.issue_date;
        if (insurance.certification_date)
          insuranceData.certification_date = insurance.certification_date;
        if (insurance.cert_valid_start)
          insuranceData.cert_valid_start = insurance.cert_valid_start;
        if (insurance.cert_valid_end)
          insuranceData.cert_valid_end = insurance.cert_valid_end;
        if (insurance.care_level)
          insuranceData.care_level = insurance.care_level;

        await cmUpsertRecord(
          "cm_kaipoke_insurance",
          insuranceData,
          { kaipoke_insurance_id: insurance.kaipoke_insurance_id },
          "被保険者証: ",
