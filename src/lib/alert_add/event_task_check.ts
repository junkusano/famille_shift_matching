...
function buildAlertMessage(task: EventTaskRow, client?: ClientRow): string {
  const name = client?.name ?? "利用者名不明";
  const csId = task.kaipoke_cs_id;

  // 既存の流儀に合わせてURLを文字列で出す（将来ページができてもOK）
  const taskUrl = `https://myfamille.shi-on.net/portal/event-tasks?task_id=${task.id}`;
  ...
  return [
    `【イベントタスク期限超過】`,
    `<a href="${taskUrl}">${name}</a>（CS ID: ${csId} / 期限: ${task.due_date} / Task: ${task.id}）`,
    clientUrl ? `利用者情報: ${clientUrl}` : "",
  ]
...
export async function runEventTaskCheck(): Promise<EventTaskCheckResult> {
  ...
  for (const t of tasks) {
    ...
    const message = buildAlertMessage(t, client);
