...
export type ShiftRecordCheckResult = {
    ok: boolean;
    checked: number;
    // 送信キュー（channelId -> message）。API側で送るために返す
    queue?: Record<string, string>;
    errors?: Array<{ code?: string; message: string; details?: string; hint?: string }>;
};

export async function runShiftRecordCheck(
  opts: { now?: Date; dryRun?: boolean } = {}
): Promise<ShiftRecordCheckResult> {
    try {
        const now = opts.now ?? new Date();
        const effectiveDryRun = opts.dryRun ?? DRY_RUN;
        ...
        if (!clientList || clientList.length === 0) {
            ...
            return { ok: true, checked: 0, queue: {} };
        }
        ...
        return { ok: false, checked: 0, errors: [{ message: String(e?.message ?? e) }] };
    }
}