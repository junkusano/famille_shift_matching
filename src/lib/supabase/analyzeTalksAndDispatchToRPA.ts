...
export async function analyzeTalksAndDispatchToRPA() {
  ...
  if (commands.length > 0) {
    await insertRpaCommandRequests(commands);
  }
}

// 1. トーク抽出
async function fetchRecentMessagesFromTalkRooms(): Promise<TalkMessage[]> {
  // TODO: LINE WORKS API で15分以内のトークを取得する処理を実装
  return [];
}
...
async function analyzeTalkMessages(messages: TalkMessage[]): Promise<RpaCommandRequest[]> {
  ...
  for (const msg of messages) {
    ...
    for (const cmd of cmds) {
      if (cmd && cmd.label && cmd.requested_date) {
        results.push({
          ...cmd,
...
// 4. シフト時間変更（stub）
function detectTimeChange(_msg: TalkMessage): Partial<RpaCommandRequest> | null {
  return null; // 今後実装
}

// 5. シフト担当変更（stub）
function detectShiftChange(_msg: TalkMessage): Partial<RpaCommandRequest> | null {
  return null; // 今後実装
}
