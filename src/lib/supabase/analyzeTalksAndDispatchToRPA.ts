...
async function expandDeletionsByWindow(
  ...
  for (const d of items) {
    ...
    if (!win) {
      // あいまい語でなければ、そのまま（toStrictDelete側でバリデートされる）
      out.push({ shift_date: date, shift_time: timeText });
      continue;
    }

    // あいまい語 → その日の該当開始時刻のシフトを全部拾う
    ...
    if (error) {
      // 取得に失敗した場合は元テキストのまま1件入れておく（後段で見え方を維持）
      out.push({ shift_date: date, shift_time: timeText });
      continue;
    }

    if (!data || data.length === 0) {
      // ヒット無しも、元テキストのままにしておく
      out.push({ shift_date: date, shift_time: timeText });
      continue;
    }

    // DBの実時刻で DeletionDetail を展開
    for (const row of data as { shift_start_time: string; shift_end_time: string | null }[]) {
      ...
      const timeLabel = et ? `${st}-${et}` : st;
      out.push({ shift_date: date, shift_time: timeLabel });
    }
  }
