...
export async function findClientsMissingContractAndPlanDocs(
  ...
  if (allDocIds.size > 0) {
    const { data: docRows, error: docError } = await supabaseAdmin
      .from("user_doc_master") // ★テーブル名＆カラム名は実際の定義に合わせて
      .select("id, label")
    ...
    if (docError) {
      ...
      for (const row of docRows ?? []) {
        const anyRow = row as { id: string; name?: string | null; label?: string | null };
        const label = anyRow.name ?? anyRow.label ?? "";
        if (anyRow.id && label) {
          docNameById[anyRow.id] = label;
        }
      }
    }
  }
