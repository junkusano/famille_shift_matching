...
export const PLAYBOOK_TEMPLATES: Array<{ key: string; label: string; summary: string; input: AgentPlaybookInput }> = [
  {
    ...
    input: {
      ...
      instructions: "対象日、開始・終了時刻、担当者、サービスコードを特定する。対象日は必ず依頼内容から確認し、日付が不明なら聞き返す。時刻・サービスコードが不足している場合は、同じ利用者様の直前のシフト（同じ曜日を優先）を参考に候補を提案する。担当者が未指定の場合は、同じ利用者様・同じサービスの直近シフトの担当者を優先し、該当者がいなければ担当マネジャー一覧の先頭を提案する。servicesuportは担当者に使用しない。補完した項目と選定元を明示する。登録内容を列記して依頼者へ確認し、依頼者がOKと答えた場合だけ、重複を再確認してシフトを追加する。追加後は結果を知らせる。",
