...
/** FAX台帳の宛先と最新の契約情報の事業所名を突合し、不一致時は送信を止める。 */
export function validateMonitoringFaxTarget(target: MonitoringFaxTarget): string | null {
  ...
  if (!registeredOffice) {
    return "契約情報の相談支援事業所／ケアマネ事業所が登録されていないため、FAX送信を停止しました。";
  }
  if (normalizeOfficeName(target.office_name) !== normalizeOfficeName(registeredOffice)) {
