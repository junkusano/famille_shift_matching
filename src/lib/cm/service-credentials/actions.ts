...
import {
  CM_OP_LOG_CREDENTIAL_CREATE,
  CM_OP_LOG_CREDENTIAL_UPDATE,
  CM_OP_LOG_CREDENTIAL_DELETE,
...
// =============================================================
// 個別取得（編集用、認証情報を含む）（読み取り専用 — 操作ログ不要）
// =============================================================
...
export async function createServiceCredential(
  ...
  try {
    ...
    return withAuditLog(
      {
        auth,
        action: CM_OP_LOG_CREDENTIAL_CREATE,
...
export async function updateServiceCredential(
  ...
  try {
    ...
    return withAuditLog(
      {
        auth,
        action: CM_OP_LOG_CREDENTIAL_UPDATE,
        resourceType: "credential",
...
export async function deleteServiceCredential(
  ...
  try {
    ...
    return withAuditLog(
      {
        auth,
        action: CM_OP_LOG_CREDENTIAL_DELETE,
        resourceType: "credential",
        resourceId: String(id),
      },
      async () => {
