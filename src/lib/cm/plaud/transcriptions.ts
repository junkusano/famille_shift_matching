...
export async function getPlaudTranscriptionList(
  ...
  try {
    ...
    logger.info("文字起こし一覧取得開始", { page, limit, status });
...
export async function executeTranscriptionAction(
  ...
  try {
    ...
    logger.info("アクション実行開始", { id, action });
...
export async function updateTranscriptionClient(
  ...
  try {
    logger.info("利用者紐付け更新開始", { id, kaipoke_cs_id: kaipokeCsId });
    ...
    // 利用者名を取得
    let clientName: string | null = null;
    if (updated.kaipoke_cs_id) {
      const { data: client } = await supabaseAdmin
        .from("cm_kaipoke_info")
        .select("name")
        .eq("kaipoke_cs_id", updated.kaipoke_cs_id)
        .single();
      clientName = client?.name ?? null;
    }

    logger.info("利用者紐付け更新完了", { id });
