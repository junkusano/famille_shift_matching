...
import { CM_OP_LOG_OTHER_OFFICE_UPDATE_FAX_PROXY } from "@/constants/cm/operationLogActions";
...
export async function updateOtherOfficeFaxProxy(
  ...
  try {
    ...
    return withAuditLog(
      {
        auth,
        action: CM_OP_LOG_OTHER_OFFICE_UPDATE_FAX_PROXY,
