...
export type SupabaseReadOnlyTableProps = {
    tableName: string;
...
export default function SupabaseReadOnlyTable({
    tableName,
    columns,
    title,
    defaultSort,
    ...
    const [sort, setSort] = useState<SortState | null>(
        defaultSort
            ? { column: defaultSort.column, ascending: defaultSort.ascending ?? true }
            : null,
    );

    const selectClause = useMemo(() => columns.map((c) => c.key).join(","), [columns]);
    ...
    useEffect(() => {
        setPage(1);
        fetchRows();
    }, [tableName, selectClause, maxRows]);
    ...
    const filteredRows = useMemo(() => {
        ...
        return rows.filter((row) => {
            const matchesGlobal =
                !globalKeyword ||
                columns.some((col) => toDisplayString(row[col.key]).toLowerCase().includes(globalKeyword));
    ...
    return (
        <div className={`rounded-2xl border border-slate-200 bg-white shadow-sm ${className}`}>
            <div className="flex flex-col gap-3 border-b border-slate-200 p-4 md:flex-row md:items-center md:justify-between">
                <div>
                    <h2 className="text-lg font-semibold text-slate-900">{title ?? tableName}</h2>
                ...
                <div className="flex flex-col gap-2 sm:flex-row">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full rounded-xl border border-slate-200 py-2 pl-9 pr-3 text-sm outline-none ring-0 placeholder:text-slate-400 focus:border-slate-400 sm:w-64"
                        />
                    </div>
            ...
            <div className="flex flex-col gap-3 border-t border-slate-200 p-4 text-sm text-slate-600 md:flex-row md:items-center md:justify-between">
                <div>
                    {sortedRows.length}件中 {(page - 1) * pageSize + 1} - {Math.min(page * pageSize, sortedRows.length)}件を表示
                </div>
