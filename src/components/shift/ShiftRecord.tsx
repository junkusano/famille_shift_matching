...
function ItemInput({ def, value, onChange, shiftInfo }: {
  ...
  // checkbox（2択=ラジオ / 3件以上=複数チェック or exclusvieでラジオ）
  if (t === "checkbox") {
    const raw = def.options ?? def.options_json;

    // 柔軟パース：{items:[...],exclusive:true} も / 単純配列 [...] も受ける
    const parsed: any = Array.isArray(raw) || typeof raw === "object" ? raw : (() => { try { return JSON.parse(String(raw)); } catch { return raw; } })();
    const exclusive: boolean = (typeof parsed === "object" && parsed && "exclusive" in parsed && !!parsed.exclusive) || !!def.exclusive;
    const opts: Array<{label: string; value: string}> = Array.isArray(parsed) ? parsed as any : Array.isArray(parsed?.items) ? parsed.items : [];

    // A) 排他 = ラジオ（N択）
    if (exclusive && opts.length >= 2) {
      const defVal = (def as any).default_value ?? (def as any).default;
    ...
    // B) 3件以上（非排他） = 複数チェック
    const optsArr: Array<{label: string; value: string}> = opts.length ? opts : parseOptionsFlexible(raw);
    if (optsArr.length >= 3) {
      const toArray = (v: unknown): string[] => {
        if (Array.isArray(v)) return v.map(String);
        if (typeof v === "string") {
          const s = v.trim(); if (!s) return [];
          try { const j = JSON.parse(s); if (Array.isArray(j)) return j.map(String); } catch {}
      ...
      const noneOpt = optsArr.find(o => String(o.label) === "該当なし" || String(o.value).toLowerCase() === "none");
      ...
      const toggle = (val: string) => {
        ...
        if (noneVal) {
          if (val === noneVal && set.has(noneVal)) { for (const o of optsArr) { const v = String(o.value); if (v !== noneVal) set.delete(v); } set.add(noneVal); }
          else { set.delete(noneVal); }
        }
    ...
    if (optsArr.length >= 2) {
      const optYes = { label: String(optsArr[0].label), value: String(optsArr[0].value) };
      let   optNo  = { label: String(optsArr[1].label), value: String(optsArr[1].value) };
...
function parseOptionsFlexible(v: unknown): { label: string; value: string }[] {
  ...
  if (typeof v === "string") {
    ...
    if (kv.every((x) => x.length === 2)) return kv.map(([k, v]) => ({ label: k, value: String(v) }));
  }
  return [];
}
