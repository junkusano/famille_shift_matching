...
export default function DefectSum() {
  ...
  async function load() {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from("biz_stats_defect_sum")
        .select("metric,snapshot_month,year_month,orgunitid,orgunitname,value,avg_3m")
  ...
  const sorted = useMemo(() => {
    // ここは既存 ShiftSum が orgs_sort で並べてるなら、同じロジックに差し替え推奨。
    // ひとまず TOTAL を末尾にするだけの最低限。
    const a = [...rows];
    a.sort((x, y) => {
      if (x.orgunitid === "TOTAL") return 1;
      if (y.orgunitid === "TOTAL") return -1;
      return x.orgunitname.localeCompare(y.orgunitname, "ja");
    });
    return a;
  }, [rows]);
