// =============================================================
// src/components/cm-components/audit/CmAuditOperationsPage.tsx
// 操作ログ一覧ページの Client Component
// /cm-portal/audit/operations のメインコンテンツ
//
// 概要:
//   cmGetTimeline で取得したタイムライン統合データを
//   CmAuditListView（フラットテーブル）で表示する。
//
// 既存コンポーネントの再利用:
//   - CmAuditFilterBar: フィルター条件カード + 凡例
//   - CmAuditListView: page_views + operation_logs の統合テーブル
// =============================================================
...
const CM_AUDIT_OPS_DEFAULT_PER_PAGE = 50;

const CM_AUDIT_OPS_INITIAL_FILTER: CmAuditLogFilter = {
  start_date: null,
  end_date: null,
  user_id: null,
  category: null,
  ...
  record_id: null,
  page: 1,
  per_page: CM_AUDIT_OPS_DEFAULT_PER_PAGE,
};
...
export function CmAuditOperationsPage() {
  // フィルター状態
  const [filter, setFilter] = useState<CmAuditLogFilter>(CM_AUDIT_OPS_INITIAL_FILTER);
  ...
  const fetchData = useCallback(async (currentFilter: CmAuditLogFilter) => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        console.error('[CmAuditOperationsPage] タイムライン取得エラー:', result.error);
      }
    } catch (error) {
      console.error('[CmAuditOperationsPage] 予期せぬエラー:', error);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleReset = useCallback(() => {
    setFilter(CM_AUDIT_OPS_INITIAL_FILTER);
    setUserFilter('');
    fetchData(CM_AUDIT_OPS_INITIAL_FILTER);
  }, [fetchData]);
