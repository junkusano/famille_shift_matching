...
export async function GET(req: NextRequest) {
    try {
        ...
        console.info('[cron][alert-reset-resigner-shift] start');
        ...
        const { data: openRows, error: openError } = await supabaseAdmin
            ...
            .like('message', '%【要修正】カイポケ削除済みスタッフがシフトに残っています%');

        if (openError) {
            console.error('[cron][alert-reset-resigner-shift] openRows error detail', {
        ...
        for (const row of openRows ?? []) {
            ...
            if (doneError) {
                console.error('[cron][alert-reset-resigner-shift] doneRow error detail', {
            ...
            if (updateError) {
                console.error('[cron][alert-reset-resigner-shift] update error detail', {
        ...
        console.info('[cron][alert-reset-resigner-shift] end', {
        ...
        console.error('[cron][alert-reset-resigner-shift] error', msg);
