...
export async function GET(req: NextRequest) {
    try {
        ...
        console.info('[cron][alert-reset-kodoengo-plan-link] start');
        ...
        const { data: openRows, error: openError } = await supabaseAdmin
            ...
            .like('message', '%【行動援護 支援手順書リンク無】%');

        if (openError) {
            console.error('[cron][alert-reset-kodoengo-plan-link] openRows error detail', {
        ...
        for (const row of openRows ?? []) {
            ...
            if (doneError) {
                console.error('[cron][alert-reset-kodoengo-plan-link] doneRow error detail', {
            ...
            if (updateError) {
                console.error('[cron][alert-reset-kodoengo-plan-link] update error detail', {
        ...
        console.info('[cron][alert-reset-kodoengo-plan-link] end', {
        ...
        console.error('[cron][alert-reset-kodoengo-plan-link] error', msg);
