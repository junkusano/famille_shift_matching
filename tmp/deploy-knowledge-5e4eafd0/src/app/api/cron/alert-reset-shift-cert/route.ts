...
export async function GET(req: NextRequest) {
    try {
        ...
        console.info('[cron][alert-reset-shift-cert] start');
        ...
        const { data: openRows, error: openError } = await supabaseAdmin
            ...
            .like('message', '%【要確認】シフト資格未整備%');

        if (openError) {
            console.error('[cron][alert-reset-shift-cert] openRows error detail', {
        ...
        for (const row of openRows ?? []) {
            ...
            if (doneError) {
                console.error('[cron][alert-reset-shift-cert] doneRow error detail', {
            ...
            if (updateError) {
                console.error('[cron][alert-reset-shift-cert] update error detail', {
        ...
        console.info('[cron][alert-reset-shift-cert] end', {
        ...
        console.error('[cron][alert-reset-shift-cert] error', msg);
