...
export async function GET(req: NextRequest) {
    try {
        ...
        console.info('[cron][alert-reset-shift-record-unfinished] start');
        ...
        const { data: openRows, error: openError } = await supabaseAdmin
            ...
            .like('message', '%【訪問記録3日以上エラー放置】%');

        if (openError) {
            console.error('[cron][alert-reset-shift-record-unfinished] openRows error detail', {
        ...
        for (const row of openRows ?? []) {
            ...
            if (doneError) {
                console.error('[cron][alert-reset-shift-record-unfinished] doneRow error detail', {
            ...
            if (updateError) {
                console.error('[cron][alert-reset-shift-record-unfinished] update error detail', {
        ...
        console.info('[cron][alert-reset-shift-record-unfinished] end', {
        ...
        console.error('[cron][alert-reset-shift-record-unfinished] error', msg);
