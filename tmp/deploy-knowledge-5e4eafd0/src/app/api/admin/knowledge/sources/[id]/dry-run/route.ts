...
export async function POST(request: NextRequest, context: Context) {
  ...
  try {
    ...
    await recordOperationLog({
      ...
      action: "knowledge.source.dry_run",
      category: "knowledge",
      description: "ナレッジ情報源のDry Runを実行",
    ...
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "Dry Runに失敗しました。" }, { status: 500 });
  }
}
