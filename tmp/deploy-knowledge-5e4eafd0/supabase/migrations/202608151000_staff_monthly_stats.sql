-- Dashboard staff trend snapshots.  The source of truth for staff identity is users.user_id.
-- NOTE: `end_at` is deliberately not a resignation-date fallback: it is also updated by
-- LineWorks synchronisation and made 113 historical resignations appear in 2026-08.

create table if not exists public.staff_monthly_stats (
  ...
  fulltime_count integer not null default 0 check (fulltime_count >= 0),
  other_count integer not null default 0 check (other_count >= 0),
  working_count integer not null default 0 check (working_count >= 0),
  calculated_at timestamptz not null default now(),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  check (fulltime_count + other_count = active_count)
);

alter table public.staff_monthly_stats enable row level security;
drop policy if exists "authenticated users can read staff monthly stats" on public.staff_monthly_stats;
create policy "authenticated users can read staff monthly stats"
  on public.staff_monthly_stats for select to authenticated using (true);

-- Rebuilds an inclusive month range.  `resign_date_latest` is the only currently
-- available, explicit resignation timestamp in this schema.  Do not substitute
-- users.end_at; it records sync activity as well as departures.
    ...
    with staff as (
      select distinct on (u.user_id)
        ...
        case when lower(btrim(coalesce(u.status, ''))) = 'removed_from_lineworks_kaipoke'
          then u.resign_date_latest::timestamptz end as retired_at,
      ...
      select distinct participant.staff_user_id
      ...
      where s.shift_start_date >= v_month
        and s.shift_start_date < (v_month + interval '1 month')::date
...
revoke all on function public.rebuild_staff_monthly_stats(date, date) from public;
